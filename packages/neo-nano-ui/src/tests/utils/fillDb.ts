...
import { Category, Comment, CommentSnapshot, Flag, Goal, Profile, Thread, Topic } from '@/lib/types/forum.types'
...
export const addCommentSnapshot = async (
  commentId: string,
...
export const addGoal = async (userId: string, overrides: Partial<Goal> = {}): Promise<string> => {
  const sql = getQueryFunction()
  const rows = await sql`INSERT INTO goals (title, target, start_date, length_days, records, user_id, metric) 
        VALUES (${overrides.title ?? ''}, ${overrides.target ?? 0}, ${overrides.startDate ?? '2000-01-01'}, ${overrides.lengthDays ?? 0}, ${overrides.records ?? new Array(overrides.lengthDays).fill(null)}, ${userId}, ${overrides.metric ?? 'words'})
        returning id`
  return rows[0].id
}
