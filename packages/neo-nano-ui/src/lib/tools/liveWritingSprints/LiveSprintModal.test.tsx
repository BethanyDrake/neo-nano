import { completePublicSprint, getMyUpcomingSprints, getPublicSprintLog } from "@/lib/serverFunctions/sprints/publicSprint"
...
describe('LiveSprintModal', () => {
  test("Alice and Bob both register, only Alice submits a wordcount", async () => {
    vi.mocked(getMyUpcomingSprints).mockResolvedValue([{id: '123', durationSeconds: 1, startTime: new Date(), visibility: 'public'}])
    ...
    const {findByText, getByRole, container, getByText} = render(<LiveSprintModal />, {wrapper: wrap(withReactQueryClient())})
