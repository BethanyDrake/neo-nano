...
export const Column = ({children, gap, style, alignItems}: Pick<Props, 'children' | 'gap' | 'style'| 'alignItems'>) => {
  return (
  <div style={{display:"flex", flexDirection:"column", alignItems, justifyContent: "center", gap: gap ?? '1em', ...(style?? {})}}>{children}</div>)
}
